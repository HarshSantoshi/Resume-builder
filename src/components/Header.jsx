  ...
  return (
    <header className="w-full flex bg-white z-50 gap-10 sticky top-0 items-center justify-between px-4 py-3 lg:px-8">
      ...
      <div className="flex-1 border rounded-md flex items-center justify-between ">
        <input
          className="flex-1 h-10 bg-transparent text-base border-none px-4 py-2 outline-slate-500 font-semibold"
          type="text"
          name="search"
          id=""
          placeholder="Search..."
          ...
          <>
            {data ? (
              <motion.div onClick={handleShow} className="relative">
                ...
                {show && 
                <AnimatePresence>
                <motion.div className="absolute px-5 py-3 rounded-md right-1 top-10 flex flex-col items-center justify-start w-60 bg-white gap-3 pt-12" onMouseLeave={()=>setShow(!show)}>
                {data?.photoURL ? (
                ...
                <div className="w-full flex-col items-center flex gap-2 pt-6">
                    <Link className="text-slate-500 hover:text-black whitespace-nowrap"  to={`/profile/${data?.uid}`}>My Profile</Link>
                    {adminIds.includes(data?.uid) && <Link className="text-slate-500 hover:text-black  whitespace-nowrap" to="/template/create">
                        Add new Template</Link>}
                </div>
                <div className="w-full px-2 py-2 border-t border-gray-300 flex items-center justify-center gap-2 group cursor-pointer" onClick={logoutUser}>
                    <p className="group-hover:text-black text-slate-500">Logout</p>
              ...
              <Link to={"/auth"}>
                <motion.button className="bg-blue-500 px-4 py-1 text-white rounded-md text-lg hover:bg-blue-800 shadow-md active:scale-95">Login</motion.button>
              </Link>
            )}
          </>
        )}
